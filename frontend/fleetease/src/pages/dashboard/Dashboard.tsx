...
const getFuelFilterValue = (
  fuelType: string,
  fuelFilters: { [key: string]: boolean }
) => {
...
const Dashboard: React.FC = () => {
  ...
  const [filteredVehicles, setFilteredVehicles] = useState<Database.Vehicle[]>(
    []
  ); // To store filtered vehicles
  ...
  const [isFilterPopoverVisible, setIsFilterPopoverVisible] =
    useState<boolean>(false);
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [searchResult, setSearchResult] = useState<Database.Vehicle | null>(
    null
  ); // Adjusted to use the Vehicle type
  ...
  const handleApplyFilter = (newFilters: any) => {
    ...
    const filtered = vehicles.filter((vehicle) => {
      // Type filters
      const typeMatch =
        (!newFilters.typeFilters.trailer &&
          !newFilters.typeFilters.bus &&
          !newFilters.typeFilters.motorcycle &&
          !newFilters.typeFilters.cargo &&
          !newFilters.typeFilters.personal &&
          !newFilters.typeFilters.special) ||
        (newFilters.typeFilters.trailer &&
          vehicle.registration_number.toLowerCase().includes("trailer")) ||
        (newFilters.typeFilters.bus &&
          vehicle.registration_number.toLowerCase().includes("bus")) ||
        (newFilters.typeFilters.motorcycle &&
          vehicle.registration_number.toLowerCase().includes("motorcycle")) ||
        (newFilters.typeFilters.cargo &&
          vehicle.registration_number.toLowerCase().includes("cargo")) ||
        (newFilters.typeFilters.personal &&
          vehicle.registration_number.toLowerCase().includes("personal")) ||
        (newFilters.typeFilters.special &&
          vehicle.registration_number.toLowerCase().includes("special"));

      // Fuel filters
      const fuelMatch =
        (!newFilters.fuelFilters.diesel &&
          !newFilters.fuelFilters.natural95 &&
          !newFilters.fuelFilters.natural98 &&
          !newFilters.fuelFilters.electric &&
          !newFilters.fuelFilters.hybrid &&
          !newFilters.fuelFilters.plugInHybrid &&
          !newFilters.fuelFilters.cng &&
          !newFilters.fuelFilters.lpg &&
          !newFilters.fuelFilters.hydrogen &&
          !newFilters.fuelFilters.ethanol &&
          !newFilters.fuelFilters.bioDiesel &&
          !newFilters.fuelFilters.syntheticFuels) ||
        (newFilters.fuelFilters.diesel && vehicle.fuel_type === "Diesel") ||
        (newFilters.fuelFilters.natural95 &&
          vehicle.fuel_type === "Natural 95") ||
        (newFilters.fuelFilters.natural98 &&
          vehicle.fuel_type === "Natural 98") ||
        (newFilters.fuelFilters.electric && vehicle.fuel_type === "Electric") ||
        (newFilters.fuelFilters.hybrid && vehicle.fuel_type === "Hybrid") ||
        (newFilters.fuelFilters.plugInHybrid &&
          vehicle.fuel_type === "Plug-in Hybrid") ||
        ...
        (newFilters.fuelFilters.bioDiesel &&
          vehicle.fuel_type === "Bio-Diesel") ||
        (newFilters.fuelFilters.syntheticFuels &&
          vehicle.fuel_type === "Synthetic Fuels");

      // State filters
      const stateMatch =
        (!newFilters.stateFilters.reserved &&
          !newFilters.stateFilters.available &&
          !newFilters.stateFilters.maintenance) ||
        (newFilters.stateFilters.reserved &&
  ...
  const handleSearch = () => {
    const result = vehicles.find(
      (vehicle) =>
        vehicle.registration_number.toLowerCase() === searchTerm.toLowerCase()
    );
